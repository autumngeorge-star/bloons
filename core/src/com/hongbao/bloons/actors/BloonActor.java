...
public class BloonActor extends RenderableActor {
	...
	public void applyStatusEffect(StatusEffectPayload payload) {
		if (bloon != null) {
			bloon.applyStatusEffect(payload);
		}
	}

	public void applyStatusEffects(List<StatusEffectPayload> payloads) {
		if (bloon != null) {
			bloon.applyStatusEffects(payloads);
		}
	}
	...
	public void act(float delta) {
		int tickDamage = bloon.updateStatusEffects(delta);
		if (tickDamage > 0) {
			if (Gdx.app != null && Gdx.app.getApplicationListener() instanceof BloonsTouhouDefense) {
				BloonsTouhouDefense app = (BloonsTouhouDefense) Gdx.app.getApplicationListener();
				app.getMap().getBloonManager().popBloon(this, tickDamage);
			} else {
				bloon.damage(tickDamage);
			}
		}
