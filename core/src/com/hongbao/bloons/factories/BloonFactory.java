...
import com.hongbao.bloons.WaveSegment;
...
public class BloonFactory {
	...
	public static BloonQueue createBloonQueueFromFile(String fileName) {
		...
		List<WaveSegment> currentLevelSegments = new ArrayList<>();
		...
		for (String line : lines) {
			if (line.startsWith("//")) {
				...
				if (parts.length == 3) {
					...
					if (bloonTypes.endsWith("\r")) {
						bloonTypes = bloonTypes.substring(0, bloonTypes.length() - 1);
					}
					
					String[] typesArray = bloonTypes.split(",");
					...
					WaveSegment segment = new WaveSegment(amount, delay, types, timer);
					currentLevelSegments.add(segment);
					timer += (long) amount * types.size() * delay;
				} else {
					System.out.println("BloonFactory.createBloonQueue(wtf2) { " + line + " }");
				}
			} else if (line.contains("END")) {
				levels.add(currentLevelSegments);
				currentLevelSegments = new ArrayList<>();
				timer = 0;
			} else {
