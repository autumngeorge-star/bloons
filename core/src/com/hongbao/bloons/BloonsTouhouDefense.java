...
public class BloonsTouhouDefense implements ApplicationListener {
	...
	private void createMenu() {
		...
		for (TowerType type : TowerType.values()) {
			ImageButton purchaseButton = new ImageButton(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal(type.getBoxImagePath())))));
			...
			Label costLabel = new Label(String.valueOf(type.getCost()), skin);
	...
	public void render() {
		if (!paused) {
			...
			for (TowerType type : TowerType.values()) {
				if (Gdx.input.isKeyJustPressed(type.getHotkey())) {
					map.setSelectedGirl(null);
					girl = GirlFactory.create(type);
					break;
				}
			}
