...
public class GameplayScreen implements Screen {
    ...
    public GameplayScreen(BloonsTouhouDefense game, MapType mapType) {
        this.game = game;
        this.mapType = mapType != null ? mapType : MapType.BASIC_MAP;
    }
    ...
    public void show() {
        ...
        autoContinue = false;
        stage = new Stage();
        player = new Player(BloonsTouhouDefense.MONEY, BloonsTouhouDefense.HEALTH);
    ...
    private void createMenu() {
        ...
        // Return to Map Selection Button
        TextButton exitButton = new TextButton("EXIT MAP", skin);
        exitButton.setPosition(1520, 20);
    ...
    public void exitToMapSelection() {
        int currentLevel = map != null && map.getBloonManager() != null ? map.getBloonManager().getLevel() : 0;
        boolean won = map != null && map.getBloonManager() != null && map.getBloonManager().hasWonGame();
        game.getProgressionService().saveProgress(mapType, currentLevel, won);
    ...
    public void render(float delta) {
        if (!paused) {
            ...
            // Auto-save progress whenever level updates or game is won
            if (map != null && map.getBloonManager() != null) {
                int level = map.getBloonManager().getLevel();
                boolean won = map.getBloonManager().hasWonGame();
                if (level > 0 || won) {
                    game.getProgressionService().saveProgress(mapType, level, won);
                }
            }
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_1)) {
                ...
                game.getMusicPlayer().toggleMusic();
