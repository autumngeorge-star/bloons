...
public class BloonManager {
	...
	public BloonManager(Stage stage, Map map) {
		this(stage, (map != null) ? map.getStageLayers() : new StageLayers(stage), map);
	}

	public BloonManager(Stage stage, StageLayers stageLayers, Map map) {
		this.stage = stage;
		this.stageLayers = (stageLayers != null) ? stageLayers : ((map != null && map.getStageLayers() != null) ? map.getStageLayers() : new StageLayers(stage));
		this.map = map;
	...
	public void createBloons() {
		...
		for (Bloon bloon : bloonsToBeCreated) {
			...
			stageLayers.addBloonActor(actor);
	...
	public void popBloon(BloonActor bloonActor, int damage) {
		...
		if (bloonActor.getBloon().willPopBloon(damage)) {
			...
			for (Bloon bloon : result.getBloonsGenerated()) {
				...
				stageLayers.addBloonActor(generatedBloonActor);
	...
	public void addBulletToStage(BulletActor bulletActor) {
		stageLayers.addBulletActor(bulletActor);
	}
